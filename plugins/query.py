...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("allfile"):
        ...
        buttons = [
            [InlineKeyboardButton("⚡Cʟɪᴄᴋ ʜᴇʀᴇ ғᴏʀ ᴍᴏʀᴇ ʙᴜᴛᴛᴏɴs⚡", callback_data="about")]
        ]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons= [[
            InlineKeyboardButton('Cʟɪᴄᴋ ᴛᴏ ᴄʟᴏsᴇ ᴛʜɪs ʙᴜᴛᴛᴏɴs', callback_data='start')
            ],[
            InlineKeyboardButton('🏠 𝙷𝙾𝙼𝙴 🏠', callback_data='help'),
            InlineKeyboardButton('🔐 𝙲𝙻𝙾𝚂𝙴 🔐', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)        
        await query.edit_message_media(
